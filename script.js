const emojiContainer = document.getElementById("joke");
const getRandomJokeBtn = document.getElementById("getRandomJokeBtn");
const dadJokes = [
    {
        "id": 1,
...
getRandomJokeBtn.addEventListener('click',()=>{
    ...
    console.log("clicked");
    let randomIndex = Math.floor(Math.random()*dadJokes.length);
    console.log(randomIndex);
        setTimeout(()=>{
            emojiContainer.innerHTML = `
            <div class ="jokeSection">
                <div >${dadJokes[randomIndex].question}</div>
                <div>${dadJokes[randomIndex].answer}</div>
            </div>`;
        },1000);
});
